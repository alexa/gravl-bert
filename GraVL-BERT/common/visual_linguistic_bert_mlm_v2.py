...
class VisualLinguisticBert(BaseModel):
    def __init__(self, config, language_pretrained_model_path=None):
        ...
        self.meta_encoder = BertEncoder(config2)
    ...
    def embedding(self,
        ...
        n_batch, n_obj, _ = object_linguistic_embeddings.size()
        ...
        # obj_vl_emb reshape [1, n_obj, 768]

        object_vl_embeddings = object_linguistic_embeddings + object_visual_embeddings

        _zero_id = torch.zeros((n_batch, ), dtype=torch.long, device=text_vl_embeddings.device)

        if text_vl_embeddings.size(1) + object_visual_embeddings.size(1) > 511:
            print('exceed max length. truncate')
            cut_len = text_vl_embeddings.size(1) + object_visual_embeddings.size(1) - 511
            vl_embeddings = torch.cat([text_vl_embeddings, object_vl_embeddings[:,:-cut_len,:], self.end_embedding(_zero_id).unsqueeze(1)], dim=1)
        else:
        ...
        grid_ind, grid_pos = torch.meshgrid(torch.arange(n_batch, dtype=torch.long, device=text_vl_embeddings.device),
        ...
        obj_type_ids = text_token_type_ids.new_zeros((n_batch, n_obj+1)).fill_(2)
        token_type_ids = torch.cat([text_token_type_ids, obj_type_ids], dim=1)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)


        # position embeddings
        txt_pos = grid_pos[:, :text_mask.size(1)] + self.position_padding_idx + 1
        if self.config.obj_pos_id_relative:
            obj_pos = txt_pos.new_zeros((n_batch, object_mask.size(1))) + (text_end + self.position_padding_idx + 1)
            end_pos = txt_pos.new_zeros((n_batch, 1)) + (text_end + self.position_padding_idx + 2)
            
        else:
            assert False, "Don't use position id 510/511 for objects and [END]!!!"
            obj_pos = txt_pos.new_zeros((n_batch, object_mask.size(1))) + (self.config.max_position_embeddings - 2)
            end_pos = txt_pos.new_zeros((n_batch, 1)) + (self.config.max_position_embeddings - 1)
        ...
        mask = torch.cat([text_mask.new_ones((n_batch, max_length-1)), text_mask.new_zeros((n_batch, 1))], dim=-1)
        ...
        meta_embeddings = self.word_embeddings_wrapper(object_meta.view(n_batch*n_obj, -1).long())
        meta_mask = object_meta.new_zeros((n_batch*n_obj, meta_embeddings.size(1))).unsqueeze(1).unsqueeze(2)
        meta_feats = self.meta_encoder(meta_embeddings, meta_mask, output_all_encoded_layers=True, output_attention_probs=False)
        meta_feats = meta_feats[-1][:, 0, :]
        meta_stream_embeddings = torch.cat([meta_feats[[0]].repeat(text_vl_embeddings.size(1), 1), meta_feats, meta_feats[[0]]], dim=0).unsqueeze(0)

        embeddings = vl_embeddings + position_embeddings + token_type_embeddings + meta_stream_embeddings
        embeddings = self.embedding_LayerNorm(embeddings)
        embeddings = self.embedding_dropout(embeddings)
        if embeddings.size(1) >= 512:
            raise ValueError('Size exceeds max limit')
        ...
        for _k, v in pretrained_state_dict.items():
            if _k.startswith('bert.') or _k.startswith('roberta.'):
                ...
                if k.startswith('encoder.'):
                    ...
                    k_ = k[len('embeddings.'):]
                    if k_ == 'word_embeddings.weight':
                        self.word_embeddings.weight.data[:v.size(0)] = v.to(dtype=self.word_embeddings.weight.data.dtype,
                                                                device=self.word_embeddings.weight.data.device)
                    elif k_ == 'position_embeddings.weight':
                        self.position_embeddings.weight.data = v.to(dtype=self.position_embeddings.weight.data.dtype,
                                                                    device=self.position_embeddings.weight.data.device)
                ...
                if k_ in self.mlm_head.predictions.state_dict():
                    base = self.mlm_head.predictions.state_dict()[k_]
                    base[:v.size(0)] = v
                    mlm_head_pretrained_state_dict[k_] = base
                else:
                    unexpected_keys.append(_k)
            else:
                unexpected_keys.append(_k)
