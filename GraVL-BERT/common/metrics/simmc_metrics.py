...
class Accuracy(EvalMetric):
    ...
    def update(self, outputs):
        with torch.no_grad():
            ...
            self.sum_metric += float((cls_preds == label).sum().item())
...
class Precision(EvalMetric):
    def __init__(self, allreduce=False, num_replicas=1):
        super(Precision, self).__init__('Prec', allreduce, num_replicas)
    ...
    def update(self, outputs):
        with torch.no_grad():
            ...
            if cls_logits.dim() == 1:
                ...
                cls_preds = cls_logits.argmax(dim=1)
            label = outputs['label'].view(-1)
            TP = (cls_preds == label) * (label == 1)
            self.sum_metric += float(TP.sum().item())
            self.num_inst += float((cls_preds == 1).sum().item())

class Recall(EvalMetric):
    def __init__(self, allreduce=False, num_replicas=1):
        super(Recall, self).__init__('Rec', allreduce, num_replicas)
    ...
    def update(self, outputs):
        with torch.no_grad():
            ...
            TP = (cls_preds == label) * (label == 1)
            self.sum_metric += float(TP.sum().item())
            self.num_inst += float((label == 1).sum().item())
