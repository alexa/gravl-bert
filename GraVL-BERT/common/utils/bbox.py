...
def coordinate_embeddings_3d(coords, dim):
    num_boxes = coords.shape[1]
    dim_mat = 1000 ** (torch.arange(dim, dtype=coords.dtype, device=coords.device) / dim)
    sin_embedding = (coords.view((num_boxes, 3, 1)) / dim_mat.view((1, 1, -1))).sin()
    cos_embedding = (coords.view((num_boxes, 3, 1)) / dim_mat.view((1, 1, -1))).cos()
    emb = torch.cat((sin_embedding, cos_embedding), dim=-1).view(num_boxes, -1)
    emb = torch.cat([emb, emb.new_zeros(emb.shape[0], 2048-emb.shape[1])], -1)
    return emb
