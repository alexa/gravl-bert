...
from common.utils.bbox import coordinate_embeddings, coordinate_embeddings_3d
...
class FastRCNN(nn.Module):
    ...
    def sample_surrounding_bbox(self, bbox, im_info):
        box_collections = []
        w0 = im_info[0, 0]
        h0 = im_info[0, 1]
        for b in range(bbox.size(1)):
            bx = bbox[0,b,:4]
            w = bx[2] - bx[0]
            h = bx[3] - bx[1]
            # top left
            tl = torch.Tensor([[bx[0]-0.5*w, bx[1]-0.5*h, bx[0], bx[1]]]).clamp(min=0)
            # top
            tp = torch.Tensor([[bx[0], bx[1]-0.5*h, bx[2], bx[1]]]).clamp(min=0)
            # tr
            tr = torch.Tensor([[bx[2], bx[1]-0.5*h, min(bx[2]+0.5*w,w0), bx[1]]]).clamp(min=0)
            # right
            rt = torch.Tensor([[bx[2], bx[1], min(bx[2]+0.5*w,w0), bx[3]]]).clamp(min=0)
            # br
            br = torch.Tensor([[bx[2], bx[3], min(bx[2]+0.5*w,w0), min(bx[3]+0.5*h, h0)]]).clamp(min=0)
            # bottom
            bm = torch.Tensor([[bx[0], bx[3], bx[2], min(bx[3]+0.5*h,h0)]]).clamp(min=0)
            # bl
            bl = torch.Tensor([[bx[0]-0.5*w, bx[3], bx[0], min(bx[3]+0.5*h,h0)]]).clamp(min=0)
            # left
            lt = torch.Tensor([[bx[0]-0.5*w, bx[1], bx[0], bx[3]]]).clamp(min=0)

            box_collections.append(torch.cat([tl, tp, tr, rt, br, bm, bl, lt], dim=0))

        box_collections = torch.cat(box_collections, dim=0).to(bbox.device)
        return box_collections

    def forward(self, images, boxes, box_mask, im_info, coord_3d=None, classes=None, segms=None, mvrc_ops=None, mask_visual_embed=None):
        """
        ...
        if self.image_feat_precomputed:
            ...
            # roi_align_res = ROIAlign(img_feats['body4'], rois, output_size=(14, 14), spatial_scale=1.0 / 16).type(images.dtype)

            su_collections = self.sample_surrounding_bbox(boxes, im_info)

            rois_s = torch.cat((
                su_collections.new_zeros(su_collections.size(0), 1),su_collections), 1)

            roi_align_su = self.roi_align(img_feats['body4'], rois_s).type(images.dtype)
            # roi_align_su = ROIAlign(img_feats['body4'], rois_s, output_size=(14, 14), spatial_scale=1.0 / 16).type(images.dtype)

            # roi_align_su[:,:,2:12,2:12] = 0
            ...
            post_roialign_su = self.head(roi_align_su)
        ...
        # coord_embed_3d = coordinate_embeddings_3d(coord_3d, 256)
        ...
        coord_embed = coordinate_embeddings(
            torch.cat((su_collections, im_info[box_inds[:, 0], :2].repeat(8,1)), 1),
            256
        )
        final_feats_su = self.obj_downsample(torch.cat((coord_embed.view((coord_embed.shape[0], -1)), post_roialign_su), -1))
        ...
        # obj_reps_su = pad_sequence(final_feats_su, box_mask.sum(1).tolist())
        obj_reps_su = final_feats_su.unsqueeze(0)
        # obj_reps_padded_su = obj_reps_su.new_zeros((obj_reps_su.shape[0], su_collections.shape[1], obj_reps_su.shape[2]))
        # obj_reps_padded_su[:, :obj_reps_su.shape[1]] = obj_reps_su
        # obj_reps_su = obj_reps_padded_su

        # Output
        output_dict = {
            'obj_reps_raw': post_roialign,
            'obj_reps': obj_reps,
            'obj_reps_su': obj_reps_su
        }
