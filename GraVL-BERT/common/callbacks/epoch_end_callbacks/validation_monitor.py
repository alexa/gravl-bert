...
class ValidationMonitor(object):
    def __init__(self, val_func, val_loader, metrics, host_metric_name='Acc', label_index_in_batch=-1, result_save_path=None):
    ...
    def __call__(self, epoch_num, net, optimizer, writer):
        self.val_func(net, self.val_loader, self.metrics, self.label_index_in_batch, self.result_save_path)

        name, value = self.metrics.get()
        if ('Prec' in name) and ('Rec' in name):
            v1 = value[name.index('Prec')]
            v2 = value[name.index('Rec')]
            f1 = 2 * v1 * v2 / (v1 + v2)
            name.append('f1')
            value.append(f1)
