...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        if 'cap' in self.data_names:
            max_cap_length = max([data[self.data_names.index('cap')].shape[1] for data in batch])
        ...
        for i, ibatch in enumerate(batch):
            ...
            if 'coords_3d' in self.data_names:
                coords_3d = ibatch[self.data_names.index('coords_3d')]
                out['coords_3d'] = clip_pad_boxes(coords_3d, max_boxes, pad=-2)

            if 'cap' in self.data_names:
                cap = ibatch[self.data_names.index('cap')]
                out['cap'] = clip_pad_2d(cap, (max_boxes, max_cap_length), pad=0)
