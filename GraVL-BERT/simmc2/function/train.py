...
def train_net(args, config):
    ...
    if args.dist:
        ...
        # model
        if num_gpus > 1:
            print('GPUs', [int(d) for d in config.GPUS.split(',')])
    ...
    # metrics
    train_metrics_list = [simmc_metrics.Accuracy(allreduce=args.dist,
    ...
    for output_name, display_name in config.TRAIN.LOSS_LOGGERS:
        train_metrics_list.append(
            simmc_metrics.LossLogger(output_name, display_name=display_name, allreduce=args.dist,
                                   num_replicas=world_size if args.dist else 1))
    ...
    validation_monitor = ValidationMonitor(do_validation, val_loader, val_metrics,
                                           ...
                                           result_save_path=args.savepath)
