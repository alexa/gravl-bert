...
from common.visual_linguistic_bert_mlm_v2 import VisualLinguisticBertForPretraining
import random
import copy
...
class ResNetVLBERT_mlm(Module):
    def __init__(self, config):

        super(ResNetVLBERT_mlm, self).__init__(config)
        ...
        self.vlbert = VisualLinguisticBertForPretraining(config.NETWORK.VLBERT,
                                         ...
                                         with_rel_head=False, with_mlm_head=True, with_mvrc_head=False)
    ...
    def train(self, mode=True):
        super(ResNetVLBERT_mlm, self).train(mode)
    ...
    def forward(self,
        ...
        mlm_label = mlm_label.long().repeat(mlm_logits.size(0), 1)
        mlm_logits_padded = mlm_logits.new_zeros((*mlm_label.shape, mlm_logits.shape[-1])).fill_(-10000.0)
        ...
        outputs.update({'mlm_logits': mlm_logits,
                        'mlm_label': mlm_label,
                        'mlm_loss': mlm_loss})

        return outputs, mlm_loss
